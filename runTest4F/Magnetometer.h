...
//    } // End of getReadings()
void getReadings() {
   mag.read();  // Attempt to read sensor data
   // Check if data is valid (optional check depending on how the library behaves)
   if (!isnan(mag.m.x) && !isnan(mag.m.y) && !isnan(mag.m.z)) {
       readings[0] = mag.m.x;
       readings[1] = mag.m.y;
       readings[2] = mag.m.z;
   } else {
